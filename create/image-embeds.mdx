...
<Tip>
  Wrap iframes in a [`<Frame>`](/components/frames) component to keep them constrained to the content column. Unwrapped iframes can overflow the text column, especially on wider layouts.
