...
<Tip>
  Wrap `iframe` elements in a [`Frame`](/components/frames) component to constrain them to the page's content column. Without `Frame`, embedded content can overflow the text area on wider viewports.
