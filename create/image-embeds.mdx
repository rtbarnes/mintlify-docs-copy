...
<Tip>
  Wrap iframes in a [`<Frame>` component](/components/frames) to constrain them to the content column and apply consistent styling. Without `<Frame>`, wide iframes can overflow the text column.
</Tip>
