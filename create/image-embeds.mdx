...
<Tip>
  Wrap iframes in a [`<Frame>` component](/components/frames) to keep them contained within the page's text column. Without `<Frame>`, iframes can overflow the column on narrower viewports.
</Tip>
