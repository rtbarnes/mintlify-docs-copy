...
Image paths are resolved relative to the root of your documentation repository (the directory that contains `docs.json`), not relative to the MDX file that references them. A leading `/` is optional but recommended for clarity.
...
Reference the image from `guides/overview.mdx` using a path from the project root:
...
<Warning>
  Relative paths like `../images/dashboard.png` may resolve during local preview but break after deployment. Always use a path from the root of your documentation repository.
</Warning>

You can also use an absolute URL to an externally hosted image:

```mdx
![Main dashboard](https://example.com/images/dashboard.png)
```
...
<Tip>
  Wrap iframes in a [`<Frame>`](/components/frames) component to keep them constrained to the content column. Without `<Frame>`, iframes can overflow horizontally on narrower layouts.
